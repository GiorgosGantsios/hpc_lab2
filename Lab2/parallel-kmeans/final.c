...

{
    ...
    do {
        ...
        #pragma omp parallel for schedule(auto) private (index, j) reduction(+:delta) reduction(+:newClusterSize[:numClusters]) reduction (+:newClusters[:numClusters*numCoords])
        for (i=0; i<numObjs; i++) {
