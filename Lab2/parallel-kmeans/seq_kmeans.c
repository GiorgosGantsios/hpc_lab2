...

{
    ...
    do {
        delta = 0.0;
        #pragma omp parallel for schedule(static) private (index, j) reduction(+:delta) reduction(+:newClusterSize[:numClusters]) reduction (+:newClusters[:numClusters*numCoords])
        for (i=0; i<numObjs; i++) {
